...
`X-Request-Id` has a plug-in: lesson 8's `CallId` reads it, or mints one,
...
hence the nullable type. Lesson 4 fixes that with property delegation.
-->
...
`StatusPages` plug-in maps it to something better (lesson 8).
-->
...
in the handler; lesson 8 turns that into a `400` with status pages.
-->
...
lesson 4 turns them into typed properties with delegation, and renders HTML
